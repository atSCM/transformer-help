...
import Transformer from 'atscm/out/lib/transform/Transformer';
import styles from 'bulma/css/bulma.min.css';
import Template from './Template.svelte';

const readJson = path => promises.readFile(path, 'utf8').then(JSON.parse);

export default class HelpTransformer extends Transformer {
  ...
  async transformFromFilesystem(node, context) {
    const { FileNode } = require(process.cwd() + '/node_modules/atscm/out/lib/gulp/src');
    const { ReferenceTypeIds } = require(process.cwd() + '/node_modules/atscm/out/lib/model/Node');
    ...
    if (node.isDisplay || node.isScript || node.isQuickDynamic) {
      ...
      const { head, html, css } = Template.render({ name: title || name, description, parameters });
